...
class TransitionManager:
    ...
    def __init__(self):
        ...
        self._transition_no_event = re.compile("\s*(\w+)\s*(-+>)\s*(\w+|\[\*\])\s*:\s*/\s*(\w+)\s*$")
    ...
    def is_transition(self, line):
        ...
        if m2:
            return Transition(m2.group(1), m2.group(3), m2.group(2), None, m2.group(4))
        m3 = self._transition.search(line)
        if m3:
            return Transition(m3.group(1), m3.group(3), m3.group(2), m3.group(4), m3.group(5))
        return None
